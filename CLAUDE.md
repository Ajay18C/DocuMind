...
2. `ExtractionRouterStage` — Strategy dispatch on `pdf_type`: `PdfType.DIGITAL` joins the already-extracted page texts; `PdfType.SCANNED` renders pages to images via the `PdfParser` and OCRs them in a worker thread via `asyncio.to_thread` (page-text extraction in `DigitalDetectionStage` is offloaded the same way). Note the thread offload keeps the loop schedulable but does NOT isolate OCR: Paddle inference holds the GIL through long native calls (measured: the loop starved 91s of a 93s OCR run), so concurrent jobs crawl until OCR finishes — the durable fix is running OCR in a separate process (own GIL), still pending. Adding a type means one new stage class plus one routing-dict entry. Unrouted types raise `UnsupportedPdfTypeError`, and an all-empty OCR result raises `EmptyOcrResultError` — both land the row in `FAILED` rather than a fake success.
